...
def visualize_graph_networkx_subplots(
    ...
    **kwargs
    ...
    # Add overall title
    fig.suptitle('Model-Dataset Graph: Multiple Layout Comparison', 
...
def visualize_graph_networkx_advanced(
    ...
    **kwargs
    ...
    if color_by == "type":
        node_colors = ['lightblue' if G.nodes[n].get("type") == MODEL else 'lightgreen' 
                      for n in G.nodes()]
    ...
    plt.title(f"Advanced Graph Visualization\nColored by {color_by}, Sized by {size_by}", 
...
def create_graph_summary_plot(
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=dataset_nodes, 
                          node_color='lightgreen', node_size=200, alpha=0.8)
    ...
    ax_components.hist(component_sizes, bins=min(20, len(component_sizes)), 
