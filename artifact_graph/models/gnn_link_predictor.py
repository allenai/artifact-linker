...
class GNNLinkPredictor(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats=1):
        ...
        self.edge_predictor = nn.Sequential(
            nn.Linear(hidden_feats * 2, hidden_feats), nn.ReLU(), nn.Linear(hidden_feats, out_feats)
        )
...
class GNNBinaryLinkPredictor(nn.Module):
    ...
    def predict_link_probability(self, x, edge_index_to_predict):
        # Get embeddings for source and destination nodes
        src_nodes = x[edge_index_to_predict[0]]
        dst_nodes = x[edge_index_to_predict[1]]
        ...
        # Predict link probability using MLP with sigmoid
        link_probability = self.edge_predictor(edge_features).squeeze(-1)
