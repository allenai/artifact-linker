...
def generate_commands(json_file: str, min_score: float = 0.7, max_score: float = 0.8, 
                     output_file: str | None = None, script_name: str = "single_eval.py"):
    """
    ...
    for pred in predictions:
        if (pred.get('status') == 'Success' and 
            'predicted_score' in pred and 
            isinstance(pred['predicted_score'], (int, float))):
    ...
    for model_info in filtered_models:
        ...
        command = f'python3 {script_name} --model "{model}" --dataset "{dataset}" --metric "{metric}"'
...
def generate_shell_script(json_file: str, min_score: float = 0.7, max_score: float = 0.8,
                         output_file: str = "batch_eval.sh", script_name: str = "single_eval.py"):
    """
    ...
    script_content.extend([
        ...
        f"echo \"Processed $total models\" | tee -a $LOG_FILE",
    ])
...
def main():
    ...
    parser.add_argument("--min-score", type=float, default=0.7, help="Minimum score threshold (default: 0.7)")
    parser.add_argument("--max-score", type=float, default=0.8, help="Maximum score threshold (default: 0.8)")
    parser.add_argument("--output", "-o", help="Output file for commands (default: print to stdout)")
    parser.add_argument("--script", default="single_eval.py", help="Script name to use (default: single_eval.py)")
    parser.add_argument("--shell-script", help="Generate executable shell script instead")
    parser.add_argument("--format", choices=['commands', 'shell'], default='commands',
    ...
    if args.format == 'shell' or args.shell_script:
        ...
        generate_shell_script(args.json_file, args.min_score, args.max_score, output_file, args.script)
    else:
