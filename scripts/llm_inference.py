...
from artifact_graph.collectors import DatasetCollector, MetricCollector, ModelCollector
from artifact_graph.models.llm_link_predictor import OpenAIGPTLinkPredictor
from artifact_graph.processors import CardProcessor, GraphBuilder
...
def evaluate_llm(predictor, data, split, node_names, G, model_cards, dataset_cards, mode="simple"):
    ...
    pred_acc = predictor.predict(
        pos_e, node_names, G=G, model_cards=model_cards, dataset_cards=dataset_cards, mode=mode
    )
...
def main(args):
    ...
    val_mse = evaluate_llm(
        predictor, data, "val", data.node_names, G, model_cards, dataset_cards, mode="simple"
    )
    print(f"Val MSE: {val_mse:.4f}")

    print("\nEvaluating on test set...")
    test_mse = evaluate_llm(
        predictor, data, "test", data.node_names, G, model_cards, dataset_cards, mode="simple"
    )
    print(f"Test MSE: {test_mse:.4f}")

    # 4) Top-50 negative-test edges
    neg_e = data.test_neg_edge_index
    neg_scores = predictor.predict(
        neg_e,
        data.node_names,
        G=G,
    ...
    names = (
        data.node_names if hasattr(data, "node_names") else [str(i) for i in range(data.x.size(0))]
    )
