...
def run_prediction_example():
    ...
    print("Command: python scripts/predict_models_for_dataset.py --dataset squad --metric accuracy --limit 5")
...
def analyze_prediction_results(results_file: str):
    ...
    if successful_predictions:
        ...
        sorted_predictions = sorted(
            successful_predictions, 
            key=lambda x: x['predicted_metric'], 
            reverse=True
        )
        ...
        for i, pred in enumerate(sorted_predictions[-5:]):
            print(f"  {len(sorted_predictions)-4+i}. {pred['model_id']}: {pred['predicted_metric']:.3f}")
