...
def check_file_coverage(
    cache_file, metadata_dir, readme_dir, entity_type="dataset", download_threshold=100
):
    """
    ...
    high_download_entities = [
        entity for entity in entities if entity.get("downloads", 0) > download_threshold
    ]
    print(
        f"Found {len(high_download_entities)} {entity_type}s with >{download_threshold} downloads"
    )
    ...
    print(f"  - Total: {total}")
    print(
        f"  - With metadata: {len(entities_with_metadata)} ({len(entities_with_metadata)/total*100:.1f}%)"
    )
    print(
        f"  - With readme: {len(entities_with_readme)} ({len(entities_with_readme)/total*100:.1f}%)"
    )
    print(f"  - With both: {len(entities_with_both)} ({len(entities_with_both)/total*100:.1f}%)")
    print(
        f"  - Missing both: {len(entities_missing_both)} ({len(entities_missing_both)/total*100:.1f}%)"
    )
    ...
    if entities_missing_both:
        ...
        for entity_id in entities_missing_both[:10]:  # Show first 10
            entity = next(
                e for e in high_download_entities if e["id"].replace("/", "__") == entity_id
            )
