    ...
    if len(valid_predictions) > 0:
        ...
        for i, result in enumerate(results_to_save[:5]):
            if result["status"] == "Success":
                ...
                print(
                    f"True: {result['true_metric']:.4f}, Predicted: {result['predicted_metric']:.4f}"
                )
                print(f"Method: {result['prediction_method']}")
                print(
                    f"Neighbors: Dataset={result['dataset_neighbors_count']}, Model={result['model_neighbors_count']}"
                )
                print("---")
    else:
