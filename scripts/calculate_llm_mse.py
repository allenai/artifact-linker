...
def print_metrics(metrics: dict):
    ...
    improvement_rmse = (
        (metrics["baseline_rmse"] - metrics["rmse"]) / metrics["baseline_rmse"]
    ) * 100
    ...
    print(
        f"Pearson Correlation: {metrics['pearson_correlation']:.6f} (p={metrics['pearson_p_value']:.6f})"
    )
    print(
        f"Spearman Correlation: {metrics['spearman_correlation']:.6f} (p={metrics['spearman_p_value']:.6f})"
    )
    print()
...
def create_scatter_plot(metrics: dict, output_dir: str | None = None):
    ...
    p = np.poly1d(z)
    plt.plot(
        metrics["true_values"], p(metrics["true_values"]), "b-", alpha=0.8, label="Best Fit Line"
    )
    ...
    if output_dir:
        ...
        plt.savefig(
            os.path.join(output_dir, "prediction_scatter_plot.png"), dpi=300, bbox_inches="tight"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate MSE and other metrics for LLM predictions"
    )
    parser.add_argument(
        "--input",
        "-i",
        type=str,
        default="./output/neighborhood_predictions_accuracy.json",
    ...
    parser.add_argument("--save-metrics", action="store_true", help="Save metrics to JSON file")
