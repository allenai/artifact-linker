...
def main():
    ...
    print(
        f"Graph for context lookup built with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges."
    )
        ...
        if not valid_predictions:
            ...
            for i in range(len(edges_to_predict)):
                ...
                if predicted_accuracies[i] is not None:
                    ...
                    print(
                        f"  - Model: {model}, Dataset: {dataset} -> Predicted: {pred_acc:.4f}, Actual: {true_acc:.4f}"
                    )
                    valid_idx += 1
                else:
