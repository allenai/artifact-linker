...
def calculate_ranking_metrics(ranked_models: List[str], neighbor_models: List[str]) -> Dict[str, float]:
...
def run(
    ...
    if max_datasets > 0:
        candidate_datasets = rng.sample(
            candidate_datasets, 
            min(max_datasets, len(candidate_datasets))
        )
    ...
    for i, dataset_name in enumerate(candidate_datasets):
        ...
        try:
            ...
            if ranking_result:
                ...
                print(f"  - Neighbors: {len(neighbor_models)}, Negatives: {len(ranking_result['negative_models'])}")
...
def parse_args():
    ...
    p.add_argument("--max-datasets", type=int, default=5, help="Maximum number of datasets to evaluate (0 for all)")
    p.add_argument("--num-negative-samples", type=int, default=5, help="Number of negative models to sample for each dataset")
    p.add_argument("--max-models-to-rank", type=int, default=10, help="Maximum number of models to rank per dataset")
    return p.parse_args()
