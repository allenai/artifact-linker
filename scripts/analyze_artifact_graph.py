...
def analyze_basic_structure(G, stats):
    ...
    analysis["edge_density"] = G.number_of_edges() / max_possible_edges if max_possible_edges > 0 else 0
...
def analyze_node_importance(G):
    ...
    analysis["top_connected_datasets"] = sorted(dataset_nodes, key=lambda x: x[1], reverse=True)[:10]
    
    # Nodes with highest downloads
    model_downloads = [(n, d.get("downloads", 0)) for n, d in G.nodes(data=True) if d.get("type") == "model"]
    dataset_downloads = [(n, d.get("downloads", 0)) for n, d in G.nodes(data=True) if d.get("type") == "dataset"]
    
    analysis["top_downloaded_models"] = sorted(model_downloads, key=lambda x: x[1], reverse=True)[:10]
    analysis["top_downloaded_datasets"] = sorted(dataset_downloads, key=lambda x: x[1], reverse=True)[:10]
...
def print_summary(basic, connectivity, importance):
    ...
    if connectivity["is_connected"]:
        ...
        print(f"  Largest Component Avg Path: {connectivity['largest_component_avg_path_length']:.2f}")
