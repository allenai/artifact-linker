...
def create_graph_structure_plot(
    G: nx.Graph, output_dir: str = "output/plots", max_nodes: int = 100, layout_type: str = "spring"
):
    ...
    if G.number_of_nodes() > max_nodes:
        ...
        sample_models = [n for n, d in sorted_nodes[: max_nodes // 2] if n in model_nodes][
            : max_nodes // 2
        ]
        sample_datasets = [n for n, d in sorted_nodes[: max_nodes // 2] if n in dataset_nodes][
            : max_nodes // 2
        ]
...
def create_degree_distribution_plot(G: nx.Graph, output_dir: str = "output/plots"):
    ...
    top_models = sorted([(n, G.degree(n)) for n in model_nodes], key=lambda x: x[1], reverse=True)[
        :10
    ]
    ...
    top_datasets = sorted(
        [(n, G.degree(n)) for n in dataset_nodes], key=lambda x: x[1], reverse=True
    )[:10]
...
def create_accuracy_distribution_plot(
    G: nx.Graph, metric_name: str = "accuracy", output_dir: str = "output/plots"
):
