#!/usr/bin/env python3
"""
One-shot smolagent: 生成一次代码，执行一次 Docker，成就成不成就不成。
基础中的基础版本，max_steps=1，无重试。
"""
...
def create_evaluation_agent(
    ...
    print(f"🔧 Tools: run_code_in_docker, read_file, save_file (ONE-SHOT MODE)")
...
def evaluate_model(
    ...
    # One-shot prompt: 简洁直接，无重试指令
    prompt = f"""
You are an expert ML engineer. Evaluate the model `{model_name}` on dataset `{dataset_name}` using metric `{metric}`.

Write a complete Python script and execute it with:
`run_code_in_docker(code, output_dir="{output_dir}")`

The script MUST:
- Load model and dataset from HuggingFace
- Use GPU: model.to("cuda")
- For large datasets (>200 examples), randomly sample up to 200
- Save results to 'results.json' as: {{"{metric}": <value>}}

Execute NOW in one step.
"""
...
def main():
    ...
    parser.add_argument("--output-dir", default="smolagent_results_oneshot")
    ...
    parser.add_argument("--max-steps", type=int, default=1, 
                        help="Max CodeAct iteration steps (default=1 for one-shot mode)")
