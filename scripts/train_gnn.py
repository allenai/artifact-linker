...
def train(model, data, optimizer, bce_loss):
    ...
    pos_pred, neg_pred = model(
        data.x, data.edge_index, data.train_pos_edge_index, neg_edge_index
    )
...
def main(args):
    ...
    data = prepare_link_pred_splits(
        nx_to_pyg_data(G), args.val_ratio, args.test_ratio
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_path', default='best_model.pth',
