...
def main():
    ...
    percentage_metrics = ["wer", "rouge-2", "rouge-l", "chrf", "bleu"]

    normalization_stats = {metric: {"total": 0, "normalized": 0} for metric in percentage_metrics}
    loss_removed_count = 0

    for edge_key, edge_data in edge_metadata.items():
        if "metrics" not in edge_data:
            continue

        # Remove loss metric completely
        if "loss" in edge_data["metrics"]:
            del edge_data["metrics"]["loss"]
            loss_removed_count += 1
            print(f"Removed loss metric from edge {edge_key}")
    ...
    print(f"Loss metrics removed: {loss_removed_count}")
