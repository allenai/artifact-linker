...
def process_readme_for_dataset(readme_path: str, output_path: str, dataset_id: str, dataset_info: dict):
...
def main():
    parser = argparse.ArgumentParser(description='Mask dataset and metric information in README files.')
    parser.add_argument('--readme_dir', type=str, default='output/models/readmes',
                        help='Directory containing README files.')
    parser.add_argument('--dataset_readme_dir', type=str, default='output/datasets/readmes',
                        help='Directory containing dataset README files.')
    parser.add_argument('--input_file', type=str, default='output/perfect_model_dataset_metrics.json',
                        help='JSON file containing dataset and metric names.')
    parser.add_argument('--output_dir', type=str, default='output/models/readmes_masked',
                        help='Directory to save the masked README files.')
    parser.add_argument('--dataset_output_dir', type=str, default='output/datasets/readmes_masked',
        ...
        for filename in os.listdir(args.readme_dir):
            if filename.endswith('.md') or filename.endswith('.txt'):
                ...
                if model_id in model_data:
                    ...
                    process_readme_for_model(readme_path, output_path, model_id, model_data[model_id])
                else:
        ...
        for filename in os.listdir(args.dataset_readme_dir):
            if filename.endswith('.md') or filename.endswith('.txt'):
                ...
                if dataset_id in dataset_data:
                    ...
                    process_readme_for_dataset(readme_path, output_path, dataset_id, dataset_data[dataset_id])
                else:
