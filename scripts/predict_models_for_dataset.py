...
def build_comprehensive_graph(graph_file: str) -> Any:
    ...
    print(
        f"🏆 Selected '{best_metric}' graph: {best_graph.number_of_nodes()} nodes, {best_graph.number_of_edges()} edges"
    )
...
def find_dataset_node(G: Any, dataset_name: str) -> Optional[str]:
    ...
    dataset_like = [
        n
        for n in all_nodes
        if any(word in n.lower() for word in ["squad", "glue", "super", "dataset"])
    ]
    ...
    for node in G.nodes():
        ...
        if node_type == "model":
            ...
            if "/" in node and len(node.split("/")) == 2:
                ...
                if not any(
                    word in node.lower()
                    for word in ["dataset", "benchmark", "test", "train", "val"]
                ):
                    models.append(node)
                else:
                    datasets.append(node)
    ...
    for i, (model, dataset) in enumerate(edges_to_predict):
        ...
        if prediction_result and prediction_result.get("prediction") is not None:
            try:
                ...
                result_item.update(
                    {"predicted_score": score, "reasoning": reasoning, "status": "Success"}
                )

            except (ValueError, TypeError) as e:
                print(f"⚠️ Failed to parse prediction for {model}: {prediction_result}")
                result_item.update(
                    {"error": f"Parse error: {e}", "raw_response": prediction_result}
                )
        else:
...
def main():
    ...
    parser.add_argument(
        "--mode", default="neighborhood", help="Prediction mode (simple, neighborhood, zero-shot)"
    )
    parser.add_argument(
        "--graph-file", default="output/perfect_model_dataset_metrics.json", help="Graph data file"
    )
    parser.add_argument("--limit", type=int, help="Limit number of models (for testing)")
    parser.add_argument(
        "--output", help="Output JSON file (default: auto-generated based on dataset and mode)"
    )
    ...
    if "error" in results:
        ...
        if results.get("top_models"):
            ...
            print(
                f"🥇 Best model: {best_model['model']} (score: {best_model['predicted_score']:.3f})"
            )
