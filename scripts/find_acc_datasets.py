            ...
            for key, value in obj.items():
                ...
                if "accuracy" in key_lower or "acc" in key_lower:
                    ...
                    if isinstance(value, (int, float)):
                        accuracy_entries.append({"path": current_path, "key": key, "value": value})
...
def find_datasets_with_accuracy(
...
def save_results(
    results: Dict[str, List[Dict[str, Any]]], output_file: str = "datasets_with_accuracy.json"
):
...
def print_summary(results: Dict[str, List[Dict[str, Any]]]):
    ...
    for dataset_name, accuracy_entries in sorted_results[:5]:  # Show top 5
        path_datasets = [
            extract_dataset_name_from_path(entry["path"]) for entry in accuracy_entries
        ]
...
def main():
    ...
    parser.add_argument(
        "--download-first", action="store_true", help="Download metrics first using MetricCollector"
    )
    parser.add_argument(
        "--dataset-names-only",
        action="store_true",
